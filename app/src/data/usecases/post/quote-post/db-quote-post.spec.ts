...
import { DbQuotePost } from './db-quote-post'

describe('DbQuotePost UseCase', () => {
  test('Should throw if PostRepository throws', async () => {
    ...
    // Act
    const result = sut.quote(quoteData)
  ...
  test('Should call PostRepository with correct values', async () => {
    ...
    // Act
    await sut.quote(quoteData)

    // Assert
    expect(createSpy).toHaveBeenCalledWith({
      message: quoteData.message,
      originalPostId: quoteData.orginalPostId,
      type: PostTypes.QUOTE,
      authorId: quoteData.authorId
    })

    expect(finByIdSpy).toHaveBeenCalledWith(quoteData.orginalPostId)
  })
  ...
  test('Should call validateMaxPostsDayByAuthor with correct values', async () => {
    ...
    // Assert
    expect(passSpy).toHaveBeenCalledWith(quoteData.authorId)
  })
  ...
  test('Should throw QuoteSelfPostError if user trying repost self post', async () => {
    // Arrange
    const { sut, postRepositoryStub } = makeSut()
    const quoteData: QuotePostModel = {
      message: 'foo bar',
    ...
    // Assert
    await expect(result).rejects.toThrow(QuoteSelfPostError)
  })

  test('Should throw QuotingQuoteTypeError if user trying repost a post of type repost', async () => {
    // Arrange
    const { sut, postRepositoryStub } = makeSut()
    const quoteData: QuotePostModel = {
      message: 'foo bar',
    ...
    // Assert
    await expect(result).rejects.toThrow(QuotingQuoteTypeError)
  })
  ...
  test('Should return an post on success', async () => {
    ...
    // Act
    const user = await sut.quote(quoteData)
...
const makeSut = (): SutTypes => {
  ...
  const sut = new DbQuotePost(postRepositoryStub, getUserStub, validateMaxPostsDayByAuthorStub)
